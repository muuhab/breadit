...
interface CommentsSesctionProps {
    postId: string
}

const CommentsSesction = async ({ postId }: CommentsSesctionProps) => {
    ...
    return <div className='flex flex-col gap-y-4 mt-4'>
        ...
        <div className="flex flex-col gap-y-6 mt-4">
            {comments.filter((comment) => !comment.replyToId).map((topLevelComment) => {
                const topLevelCommentsVotesAmt = topLevelComment.votes.reduce((acc, vote) => {
...
export default CommentsSesction