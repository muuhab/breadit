            ...
            if (index === posts.length - 1) {
                return (
                    <li key={post.id} ref={ref}>
                        <Post votesAmt={votesAmt} currentVote={currentVote} subredditName={post.subreddit.name} post={post} commentAmt={post.comments.length} />
                    </li>
                )
            } else {
                return <Post votesAmt={votesAmt} currentVote={currentVote} subredditName={post.subreddit.name} post={post} commentAmt={post.comments.length} key={post.id} />
            }
        })}
        {isFetchingNextPage && (
